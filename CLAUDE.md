...
```bash
# Initial schema + migrations (run in order, 001–019)
...
# 019 geo structured conditions (effect/conditions columns on geographic_restrictions)
...
Pure MySQL queries: filters `ltv_matrix` by LTV/CLTV, loan amount, FICO, DTI/DSCR, occupancy, purpose, doc type → applies `geographic_restrictions` and `credit_event_seasoning` → returns matched programs with `rag_notes` and `special_overlay`. Accepts `quick=True` to skip Qdrant note fetching.
...
- `geographic_restrictions`, `credit_event_seasoning`, `program_requirements` — overlay rules

...
from chatbot_retrieval.eligibility_tolerance import (
    ELIG_DSCR_TOLERANCE,
    ELIG_FICO_TOLERANCE,
    ELIG_LOAN_TOLERANCE,
    ELIG_PCT_TOLERANCE,
...
from chatbot_retrieval.summarize_notes import filter_notes_for_summarize  # noqa: E402

from chatbot_retrieval.eligibility_trace import (  # noqa: E402
    LAYER10,
    LAYER2,
    LAYER3,
    LAYER4B,
...
def _extract_years(value: str) -> float | None:
    ...
    Bucket strings come from the frontend: "<2 years", "2-3 years", "4+ years", "7+ years".
    ...
    if not value:
        return None
    s = value.strip()
    # "<2 years" — treat conservatively as just-under so tight seasoning gates are not wrongly passed
    if re.search(r"<\s*2\s+year", s, re.IGNORECASE):
        return 1.0
    # "N+ years" — "4+ years", "7+ years"
...
def _dscr_band_ok(band: str | None, dscr: float | None) -> bool:
    ...
    if dscr is None:
        return True
    if b == "gte_1_00":
        return dscr >= 1.0 - ELIG_DSCR_TOLERANCE
    if b == "gte_0_75_lt_1_00":
        return dscr >= 0.75 - ELIG_DSCR_TOLERANCE
    if b == "lt_0_75":
        return dscr < 0.75 + ELIG_DSCR_TOLERANCE
    return True
...
def _layer2_ltv_matrix(
    ...
    params: dict[str, Any] = {
        "ids": program_ids,
            "fico": form["fico"],
        "fico_for_min": (
            int(form["fico"]) + ELIG_FICO_TOLERANCE
            if form["fico"] is not None
            else None
        ),
    }
...
def _pick_scenario_matrix_row(
    ...
    if fico is not None:
        qualifying = [
            r for r in scoped
            if fico_meets_min(int(fico), int(r.get("fico_min") or 0))
        ]
        if qualifying:
    ...
    if loan_amount is not None:
        ...
        ceilings = [
            ...
            and loan_within_tier(amt, r.get("loan_amt_min"), r.get("loan_amt_max"))
        ]
        if ceilings:
...
def _ltv_from_row_for_purpose(
    ...
    return _to_float_opt(row.get(cap_key))
...
def _evaluate_near_miss(
    ...
    if not ltv_rows:
        ...
        if not tier_rows:
            ...
            if max_tiers and exceeds_loan(amt, max(max_tiers)):
                ...
                qualifying_mins = [m for m in min_tiers if below_loan(amt, m)]
                if qualifying_mins:
            ...
            if (
                ...
                or not exceeds_loan(int(loan_amount), int(scenario_max))
            ):
                return None
...
def _find_near_miss_programs(
    ...
    *,
    ...
    """Up to `limit` programs that miss only on LTV or loan amount (other filters held)."""
    if quick or limit <= 0:
        return []
...
def _layer3_fthb(
    ...
    for cand in candidates:
        ...
        if cap is not None and loan_amount is not None and exceeds_loan(loan_amount, int(cap)):
            continue
...
def _is_unknown_geo_subfield(value: str) -> bool:
    """County/city is generic or missing — don't apply county-specific hard blocks."""
    v = (value or "").strip().lower()
    if not v:
        return True
    if v in ("other", "unknown", "n/a", "not sure", "unsure"):
        return True
    return v.startswith("other")  # e.g. "other / not listed"


def _full_geo_block_applies(detail: str, form: dict[str, Any]) -> bool:
    """
    For ineligible geographic rows, check whether the restriction's
    sub-location condition actually matches the form's provided location.
    If the user hasn't specified the relevant sub-location (city/county/zip)
    don't block — be optimistic until they do.
    """
    lower = detail.lower()
    state = (form.get("state") or "").upper()
    county = (form.get("county") or "").strip().lower()
    city = (form.get("city") or "").strip().lower()
    zip_code = (form.get("zip_code") or "").strip()

    # Territories — pure state-code check, always apply
    if any(t in lower for t in ("puerto rico", "guam", "us virgin islands", "u.s. virgin islands")):
        return state in ("PR", "GU", "VI")

    # "Ineligible States: TX" — pure state-level hard block
    m = re.search(r"ineligible states?:\s*([A-Z]{2})", detail, re.IGNORECASE)
    if m:
        return state == m.group(1).upper()

    # Zip-code specific suspension
    if "zip code" in lower:
        zips = re.findall(r"\b(\d{5})\b", detail)
        if zips:
            return bool(zip_code) and zip_code in zips
        return False

    # Baltimore City
    if "baltimore city" in lower:
        if form.get("is_in_baltimore_city"):
        return True
        return bool(city) and "baltimore" in city

    # Philadelphia County
    if "philadelphia county" in lower or ("philadelphia" in lower and "county" in lower):
        if form.get("is_in_philadelphia"):
            return True
        return bool(county) and "philadelphia" in county

    # Cook County (IL)
    if "cook county" in lower:
        return bool(county) and "cook" in county

    # Memphis, TN
    if "memphis" in lower:
        is_in_mem = str(form.get("is_in_memphis", "")).lower() in {"yes", "true", "1"}
        city_val = (form.get("city") or "").lower()
        return is_in_mem or "memphis" in city_val

    # Generic county/city restriction — parse named locations and match precisely.
    # "Other" or empty county/city means unknown → be optimistic (don't block).
    # Note: match both "county" and "counties" (the latter doesn't contain "county" as a substring).
    _has_county_word = bool(re.search(r'count(?:y|ies)', lower))
    _has_city_word = bool(re.search(r'cit(?:y|ies)', lower))

    if _has_county_word or _has_city_word:
        if _has_county_word:
            # Extract comma/and-separated list before "County" or "Counties"
            # e.g. "Charlotte, Lee, Hendry, and Glades Counties:"
            m_list = re.search(
                r'([A-Za-z][A-Za-z, ]+)\s+Count(?:y|ies)\b',
            ...
            if m_list:
                ...
                named = [
                    n.strip().lower()
                    for n in re.split(r',|\band\b', raw, flags=re.IGNORECASE)
                    if n.strip()
                ]
                if named:
                    if _is_unknown_geo_subfield(county):
                        return False  # county unknown — optimistic
                    return any(n in county.lower() or county.lower() in n for n in named)
            # No named counties parsed — only block when county is a real known value
            return not _is_unknown_geo_subfield(county) and bool(county)

        if _has_city_word:
            m_list = re.search(
                r'([A-Za-z][A-Za-z, ]+)\s+Cit(?:y|ies)\b',
                detail, re.IGNORECASE,
            )
            if m_list:
                raw = m_list.group(1)
                ...
                if named:
                    if _is_unknown_geo_subfield(city):
                        return False
                    return any(n in city.lower() or city.lower() in n for n in named)
            return not _is_unknown_geo_subfield(city) and bool(city)

    # Can't determine sub-location condition — apply the block
    return True


def _is_geo_ineligible_restriction(rtype: str) -> bool:
    """True when the row hard-blocks the program (vs LTV cap / informational overlay).
    full_ineligibility_criteria  = always blocks (hard remove).
    partial_ineligibility_criteria = conditional — surfaces as overlay note only.
    ineligibility_criteria kept as legacy alias for full.
    """
    t = (rtype or "").strip().lower()
    return t in ("full_ineligibility_criteria", "ineligibility_criteria", "ineligible", "ineligible_state")
...
def _fetch_eligible_state_allowlists(
    conn: Any,
    program_ids: list[int],
    lender_ids: list[int],
) -> tuple[dict[int, set[str]], dict[int, set[str]]]:
    """
    Positive allowlists from map_geographic_restrictions (restriction_type = eligible_state).
    Returns (by_lender_id, by_program_id). Program-specific rows override lender-wide rows.
    """
    if not program_ids:
        return {}, {}

    lids = sorted({int(x) for x in lender_ids if x})
    q = text(
        """
        SELECT lender_id, program_id, state
        FROM map_geographic_restrictions
        WHERE restriction_type = 'eligible_state'
          AND (
            program_id IN :ids
            OR (program_id IS NULL AND lender_id IN :lids)
          )
        """
    ).bindparams(bindparam("ids", expanding=True), bindparam("lids", expanding=True))

    try:
        rows = conn.execute(q, {"ids": program_ids, "lids": lids or [0]}).fetchall()
    except Exception:
        return {}, {}

    by_lender: dict[int, set[str]] = {}
    by_program: dict[int, set[str]] = {}
    for row in rows:
        r = dict(row._mapping)
        ...
        lid = r.get("lender_id")
        if pid is not None:
            by_program.setdefault(int(pid), set()).add(st)
        elif lid is not None:
            by_lender.setdefault(int(lid), set()).add(st)
    return by_lender, by_program


def _state_eligibility_block_reason(
    cand: dict[str, Any],
    state: str,
    eligible_by_lender: dict[int, set[str]],
    eligible_by_program: dict[int, set[str]],
) -> str | None:
    """Hard block when a lender/program publishes an eligible-state allowlist and state is not on it."""
    cpid = int(cand["program_id"])
    clid = int(cand.get("lender_id") or 0)
    allowlist = eligible_by_program.get(cpid)
    if allowlist is None:
        allowlist = eligible_by_lender.get(clid)
    if allowlist is None:
        return None
    st = (state or "").strip().upper()
    if st not in allowlist:
        return f"{st} is not an eligible lending state for this program"
    return None
...
def _layer5_geo(
    ...
    geo_blocked: program_id → reason string
    overlay_notes: program_id → note string
    """
    if not candidates or not form.get("state"):
        for c in candidates:
    ...
    ids = [int(r["program_id"]) for r in candidates]
    state = form["state"]
    lender_ids = [int(c.get("lender_id") or 0) for c in candidates]

    geo_blocked: dict[int, str] = {}
    overlay_notes: dict[int, str] = {}

    # Step 1 — state allowlist (eligible_state rows) before county/city overlays
    eligible_by_lender, eligible_by_program = _fetch_eligible_state_allowlists(
        conn, ids, lender_ids
    )
    state_gated: list[dict[str, Any]] = []
    for cand in candidates:
        cpid = int(cand["program_id"])
        reason = _state_eligibility_block_reason(
            cand, state, eligible_by_lender, eligible_by_program
        )
        if reason:
            geo_blocked[cpid] = reason
        else:
            state_gated.append(cand)

    if not state_gated:
        return [], geo_blocked, overlay_notes

    gated_ids = [int(c["program_id"]) for c in state_gated]

    q = text(
        """
        SELECT program_id, lender_id, restriction_type, restriction_detail
        FROM map_geographic_restrictions
        WHERE state = :state
    ...
    rows = conn.execute(q, {"state": state, "ids": gated_ids}).fetchall()

    # Build a set of (lender_id → program_ids) for NULL-program lender-wide blocks
    lender_to_pids: dict[int, list[int]] = {}
    for cand in state_gated:
        lid = int(cand.get("lender_id") or 0)
        lender_to_pids.setdefault(lid, []).append(int(cand["program_id"]))

    for row in rows:
        r = dict(row._mapping)
        pid = r.get("program_id")
        lid = r.get("lender_id")
        rtype = (r.get("restriction_type") or "").strip().lower()
        rdetail = (r.get("restriction_detail") or "").strip() or rtype

        # Determine target program ids
        if pid is not None:
            targets = [int(pid)]
        elif lid is not None:
            # Lender-wide restriction
            targets = lender_to_pids.get(int(lid), [])
        else:
            targets = gated_ids

        if rtype == "eligible_state":
            continue

        if _is_geo_ineligible_restriction(rtype):
            for t in targets:
                if not _full_geo_block_applies(rdetail, form):
                    # Sub-location unknown or doesn't match — surface as note, don't hard-block
                    existing = overlay_notes.get(t, "")
                    overlay_notes[t] = (existing + " | " + rdetail).lstrip(" | ") if existing else rdetail
                else:
                geo_blocked[t] = rdetail
        elif rtype == "partial_ineligibility_criteria":
            # Conditional restriction — keep program but surface as an overlay caution
            for t in targets:
                existing = overlay_notes.get(t, "")
                overlay_notes[t] = (existing + " | " + rdetail).lstrip(" | ") if existing else rdetail
        elif rtype == "ltv_cap":
            for t in targets:
                overlay_notes[t] = f"LTV cap in {state}: {rdetail}"
        else:
            for t in targets:
                overlay_notes[t] = rdetail
    ...
    for cand in state_gated:
        ...
        if cpid in geo_blocked:
            continue
        note = overlay_notes.get(cpid)
        if note:
            cand["special_overlay"] = note
        else:
...
def _parse_credit_events(raw_event_str: str) -> list[tuple[str, float]]:
    ...
    Handles inputs like:
      - "BK-Ch7-Discharged 4+ years"          → [("bk", 4.0)]
      - "FC; SS 2-3 years"                     → [("fc", ?), ("ss", 2.0)]
      - "BK-Ch7-Discharged 4+ years; SS 2-3 years" → [("bk", 4.0), ("ss", 2.0)]
...
def _citizenship_npra_block_reason(
    ...
    if npra_max is not None:
        ...
        if leverage is not None and exceeds_pct(leverage, float(npra_max)):
...
def _acreage_block_reason(
    ...
    if exceeds_acreage(float(acres), float(cap)):
...
def _layer10_qdrant_verify(
    ...
    try:
        from qdrant_client import QdrantClient  # optional dep — may not be installed
        client = QdrantClient(url=config.QDRANT_URL, prefer_grpc=False, timeout=15)
    except Exception:
        return candidates, []
...
def find_eligible_programs(
    ...
    if not quick and len(candidates) < 3 and prog_by_id:
